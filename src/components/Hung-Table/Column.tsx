  ...
  return (
    <div className=" w-full">
      <div className="m-auto px-5 pb-8 pt-5 @lg:pt-6 @2xl:px-7 ">
        ...
        <p className="text-lg text-blue-600">Description: {props.data.ticket}</p>
        <p className="text-lg text-red-600">Comments:</p>
        <div className=" border-orange-300 border-2 p-3 ">
          {props.data.comments.length === 0 ? <p>Don't have any comments</p> :
            <div>
              {props.data.comments.map((item: any) => (
                <figure className="flex gap-5 mb-3">
                  <div className="flex w-10 h-10  center  bg-gray-100 rounded-full dark:bg-gray-600">
                    <span className="font-medium  text-center align-middle w-10 h-10 p-1.5 text-gray-600 dark:text-gray-300">JL</span>
                  </div>
                  <div>
                    <p className="font-semibold">{item.username} <span className=" font-normal italic"> (
...
export const getColumns = ({
  ...
  openModal,
    ...
    {
      title: (
        <div className="ps-2 h-50">
          #
        </div>
      ),
      dataIndex: "seq",
      key: "seq",
      render: (_: any, row: any, count: any) => (
        <div className="inline-flex ps-2 text-white">
          {count + 1}
        </div>
      ),
    },
...

    {
      ...
      dataIndex: "servity",
      key: "servity",
      render: (servity: string) => <div className=" font-semibold">
        {servity==='Low'?<p className=" text-green-500">Low</p>:(servity==='Medium'?<p className=" text-yellow-400">Medium</p>:<p className=" text-red-500">High</p>)}
      </div>,
    }
    ,
...
export function DateCell({
  ...
  return (
    <div className={cn(className, "grid gap-1")}>
      <time dateTime={formatDate(date, "YYYY-MM-DD")} className={cn("font-medium text-white", dateClassName)}>
        {formatDate(date, dateFormat)}
      </time>
      <time dateTime={formatDate(date, "HH:mm:ss")} className={cn("text-[13px] text-white", timeClassName)}>
