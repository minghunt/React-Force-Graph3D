...
export const getColumns = ({
  ...
  },
  {
    ...
    render: (_: string, row: any) => <AvatarCard src={row.avatar} name={row.userName} description={`HPT-${row.id}`} />,
  },
  {
  ...
  },
  {
    ...
    render: (_: string, row: any) => (
      <div className="flex items-center justify-start gap-3 pe-3">
        <Tooltip size="sm" content={() => "Edit Invoice"} placement="top" color="invert">
          <ActionIcon
            onClick={() => {
              openModal({
        ...
        <Tooltip size="sm" content={() => "View Invoice"} placement="top" color="invert">
  ...
  return (
    <figure className={cn("flex items-center gap-3", className)}>
      ...
      <figcaption className="grid gap-0.5">
        <div className="font-lexend text-sm font-medium text-gray-900 dark:text-gray-700">{name}</div>
        {description && <div className="text-[13px] text-gray-500">{description}</div>}
      </figcaption>
    </figure>
  );
}
...
export function EyeIcon({ strokeWidth, ...props }: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
...
export function PencilIcon({ strokeWidth, ...props }: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
